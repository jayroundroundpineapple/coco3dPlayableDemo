...
export class Board extends Component {
    ...
    initDiamond() {
        for (let i = 0; i < 5; i++) {
            ...
            this.node.parent!.addChild(this.diamondList[i]);  //修改点
            this.diamondList[i].active = false;
        }
    }
            ...
            for (let i = 0; i < 5; i++) {
                if (this.diamondList[i].active) {
                    ...
                    if (Math.abs(x - this.diamondList[i].position.x) <= Constants.DIAMOND_SCORE_AREA) {
                        ...
                        this.hideDiamond(i);
                        Constants.game.addScore(Math.floor(LanguageManager.instance.formatUnit(Constants.DIAMOND_SCORE)));
                        Constants.game.audioManager.playClip1(RESSpriteFrame.instance.getCoinAudioClip)
                    }
                }
            }
            if (flag) {
    ...
    setWave(isCenter:boolean = false) {
        if (this.type != Constants.BOARD_TYPE.GIANT) {
            ...
            // this.wave.children[0].active = isCenter
