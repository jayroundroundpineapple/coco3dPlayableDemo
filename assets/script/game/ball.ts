...
export class Ball extends Component {
        ...
        if (Constants.game.state === Constants.GAME_STATE.PLAYING) {
            ...
            if (this.jumpState === Constants.BALL_JUMP_STATE.SPRINT) {
                ...
                for (let i = this.currBoardIdx + 1; i >= 0; i--) {
                    ...
                    //超过xx分，显示congratulations
                    if(Constants.game.score >= LanguageManager.instance.formatUnit(300)){
                        ParticleUtils.stop(this.trailNode!);
                            Constants.game.gameSuccess();
                            return;
                    }
                    // 超过当前跳板应该弹跳高度，开始下降
    ...
    activeCurrBoard() {
        ...
        if (!this.currBoard.isActive) {
            ...
            this.isCenter = false
            let score = Math.floor(LanguageManager.instance.formatUnit(Constants.SCORE_BOARD_NOT_CENTER));  //跳板中心得分翻倍
            if (boardType !== Constants.BOARD_TYPE.NORMAL && boardType !== Constants.BOARD_TYPE.DROP || Math.abs(pos.x - boardPos.x) <= Constants.BOARD_RADIUS_CENTER) {
                score = Math.floor(LanguageManager.instance.formatUnit(Constants.SCORE_BOARD_CENTER))
                this.isCenter = true
            }
        ...
        this.currBoard.setWave(this.isCenter);
    ...
    newBoard() {
        ...
        if (this.boardGroupCount <= 0) {
            ...
            if (t < 4.2) {
                ...
                this.boardGroupCount = 3
            } else if (false) {  //t <= 7.5 && false === this.hasSprint
    ...
    showScore(score: number) {
        ...
        node.getComponentInChildren(Label)!.string = `+${LanguageManager.instance.getText(10001)}${score}`;
